...
class TaskFilterUtils {
  Future<List<TaskDTO>> filterTasks(List<TaskDTO> allTasks, TaskFilter? filterValue) async {
    ...
    switch (filterValue) {
      case TaskFilter.today:
        return _filterByToday(allTasks);
      case TaskFilter.tomorrow:
        return _filterByTomorrow(allTasks);
      default:
        return allTasks;
    }
  }

  Future<List<TaskDTO>> _filterByToday(List<TaskDTO> tasks) async {
    try {
  ...
  Future<List<TaskDTO>> _filterByTomorrow(List<TaskDTO> tasks) async {
    try {
      if (tasks.isEmpty) return [];
