...
import 'package:uptodo/widgets/task_detail/components/delete_confirmation_dialog.dart';
import 'package:uptodo/widgets/task_detail/components/edit_task_name_dialog.dart';
...
class _TaskDetailScreenState extends State<TaskDetailScreen> {
  ...
  void _setTaskCompleted(String userId, TaskDTO task) {
    setState(() {
      _currentTask = task.copyWith(isCompleted: !_currentTask.isCompleted!);
    });
    Provider.of<TaskProvider>(context, listen: false).updateTask(userId, _currentTask);
    TopNotification.showSuccess(context, 'Task marked as ${_currentTask.isCompleted! ? 'completed' : 'not completed'}');
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  SizedBox(
                    child: IconButton(
                      onPressed: () {
                        final userId = authProvider.currentUser?.id ?? '';
                        _setTaskCompleted(userId, _currentTask);
                      },
                      icon: Icon(
              ...
              const SizedBox(height: 16),
              if (_currentTask.isSubtask != true || _currentTask.isSubtask == false)
                _buildDetailRow(
  ...
  Future<void> _showSubTaskPicker() async {
    ...
    setState(() {
      _subTasks = selectedSubTasks?.map((task) => task.copyWith(isSubtask: true)).toList();
    });
    widget.onTaskUpdated?.call();
  }
}
